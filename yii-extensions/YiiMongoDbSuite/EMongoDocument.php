...
abstract class EMongoDocument extends EMongoEmbeddedDocument
{
	...
	public function getMongoDBComponent()
	{
		if(self::$_emongoDb===null)
			self::$_emongoDb = Yii::app()->getComponent($this->getMongoDBComponentName());
		...
		if($this->beforeSave())
		{
			...
			if(version_compare(Mongo::VERSION, '1.0.5','>=') === true)
				$result = $this->getCollection()->insert($rawData, array(
					...
					// changed by Plexisoft: 'safe' was replaced by 'w'
					'w'	=> $this->getSafeFlag()
				));
			else
	 ...
	 * @since v1.0
	 */
	// Plexisoft: we implemented version numbering for each document in the database to implement transaction-style write operations;
	//            see further insertions of our code with appropriate comments;
	//            note, that in updateAll() method we did not implement this feature
	public function update(array $attributes=null, $modify = false)
	{
		...
		if($this->beforeSave())
		{
			...
			if($attributes!==null)
			{
				...
				foreach($rawData as $key=>$value)
				{
					if ($key == "versionNumber") continue; // added by Plexisoft
					if(!in_array($key, $attributes))
						unset($rawData[$key]);
				}
			}

			$rawData["versionNumber"]++; // added by Plexisoft
			
			// commented by Plexisoft
			/*
			if($modify)
			{
			*/
				...
				$result = $this->getCollection()->update(
					array('_id' => $this->_id, 
						'versionNumber' => ($rawData["versionNumber"] - 1)), // added by Plexisoft
					array('$set' => $rawData),
					array(
						'w' => 1, // added by Plexisoft
						'fsync'=>$this->getFsyncFlag(),
						// changed by Plexisoft: 'safe' was replaced by 'w'
						'w'=>$this->getSafeFlag(),
						'multiple'=>false
				...
				if ($result["n"] != 1)
				{
					$this->addError ("_id", t("Failed to update the document in the database. Probably it was modified by another system user or process. Please, try again."));
					return false;
				}
				else
				{
	 ...
	 * @since v1.2.2
	 */
	// customized by Plexisoft: private replaced with protected
